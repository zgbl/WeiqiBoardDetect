...
class HybridScannerV4:
    ...
    def detect(self, img):
        print("\n" + "="*50)
        print("  Hybrid Scanner V3 (Enhanced OpenCV Engine)")
        print("="*50)
        h, w = img.shape[:2]
        ...
        for i, pt in enumerate(corners):
            ...
            exact_pt = self.exact_recognize_corner(img, pt, tag, angle1, angle2, debug_canvas)
    ...
    def exact_recognize_corner(self, img, approx_pt, corner_type, angle1, angle2, debug_canvas=None):
    ...
    def _extract_precise_opencv_l_shape(self, img, patch_center, corner_type, angle1, angle2, debug_canvas=None, roi_r=100):
        ...
        if debug_canvas is not None:
            cv2.rectangle(debug_canvas, (x1, y1), (x2, y2), (255, 0, 255), 2)
            cv2.imshow(f"{corner_type} OpenCV ROI (Original)", roi)
        
        # 1. 预处理：灰度 -> 高斯模糊 -> CLAHE 增强
        gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(gray, (9, 9), 0)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        enhanced = clahe.apply(blur)
        ...
        print(f"  [OpenCV L-Shape] Found {len(h_rhos)} H-lines and {len(v_rhos)} V-lines near center.")
        ...
        # ── Step 1: Hough 求 L 形交点（取排序后的第二根线） ──────────────────
        ...
        hough_intersect = intersect_lines(best_h[0], best_h[1], best_v[0], best_v[1])
        if hough_intersect is None: return patch_center
        hx, hy = hough_intersect

        # ── Step 2: Harris 在局部 ROI 内提取角点 ──────────────────────────
        h_dst = cv2.cornerHarris(enhanced, blockSize=4, ksize=3, k=0.04)
        h_dst = cv2.dilate(h_dst, None)
        harris_thresh = 0.01 * h_dst.max() if h_dst.max() > 0 else 1.0 
        ry_pts, rx_pts = np.where(h_dst > harris_thresh)

        if len(rx_pts) == 0:
            final_pt = hough_intersect
        else:
            harris_global = np.column_stack([rx_pts + x1, ry_pts + y1]).astype(float)
            # ── Step 3: Snap 逻辑，寻找离 Hough 最近的 Harris 候选点 ──────────
            SEARCH_R = 20  
            dists = np.sqrt((harris_global[:, 0] - hx)**2 + (harris_global[:, 1] - hy)**2)
            min_idx = np.argmin(dists)

            if dists[min_idx] < SEARCH_R:
                final_pt = (harris_global[min_idx, 0], harris_global[min_idx, 1])
                print(f"  [Harris] Snapped: Hough({hx:.0f},{hy:.0f}) -> Harris({final_pt[0]:.0f},{final_pt[1]:.0f}), d={dists[min_idx]:.1f}px")
            else:
                final_pt = hough_intersect
                print(f"  [Harris] No reliable snap within {SEARCH_R}px.")

        # ── 可视化 ───────────────────────────────────────────────────
        if debug_canvas is not None:
            # 最终 Pinned 点用紫色圆圈
            cv2.circle(debug_canvas, (int(final_pt[0]), int(final_pt[1])), 8, (255, 0, 255), -1)
            cv2.circle(debug_canvas, (int(final_pt[0]), int(final_pt[1])), 10, (255, 255, 255), 1)

            roi_result = roi.copy()
            local_ix, local_iy = int(final_pt[0] - x1), int(final_pt[1] - y1)
            cv2.circle(roi_result, (local_ix, local_iy), 6, (0, 0, 255), -1)
            cv2.circle(roi_result, (local_ix, local_iy), 7, (255, 255, 255), 1)
            cv2.putText(roi_result, f"Pinned {corner_type}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.imshow(f"{corner_type} OpenCV ROI (Pinned Result)", roi_result)
