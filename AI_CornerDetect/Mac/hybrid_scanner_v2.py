...
class HybridScannerV2:
    ...
    def detect(self, img):
        print("\n" + "="*50)
        print("  Hybrid Scanner V2 (Enhanced OpenCV Engine)")
        print("="*50)
        h, w = img.shape[:2]
    ...
    def exact_recognize_corner(self, img, approx_pt, corner_type, angle1, angle2, debug_canvas=None):
        ...
        final_pt = self._extract_precise_opencv_l_shape(img, (cx, cy), corner_type, angle1, angle2, debug_canvas)
    ...
    def _extract_precise_opencv_l_shape(self, img, patch_center, corner_type, angle1, angle2, debug_canvas=None, roi_r=100):
        ...
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        gray = clahe.apply(gray)
        if debug_canvas is not None: cv2.imshow(f"{corner_type} OpenCV ROI (CLAHE)", gray)
        
        # 用较低的阈值提高获取线的成功率
        edges = cv2.Canny(gray, 30, 100)
        if debug_canvas is not None: cv2.imshow(f"{corner_type} OpenCV ROI (Canny)", edges)
        
        lines = cv2.HoughLinesP(edges, 1, np.pi/180, 15, minLineLength=15, maxLineGap=10)
        if lines is None: 
        ...
        for seg in lines:
            ...
            # 强化约束：必须严格平行于全盘提取出的主角度 angle1(横向) 和 angle2(纵向)！容差15度
            if abs(dx) > abs(dy):
        ...
        # 定位L形状的最外围相交线 (以Y/X方向的最值判断最外缘)
        if 'T' in corner_type: best_h = min(h_rhos, key=lambda item: item[1])[0]
        else: best_h = max(h_rhos, key=lambda item: item[1])[0]
            
        if 'L' in corner_type: best_v = min(v_rhos, key=lambda item: item[1])[0]
        else: best_v = max(v_rhos, key=lambda item: item[1])[0]
        ...
        if intersect:
            if debug_canvas is not None:
                cv2.circle(debug_canvas, (int(intersect[0]), int(intersect[1])), 8, (255, 0, 255), -1)
            return intersect
        else:
            return patch_center
