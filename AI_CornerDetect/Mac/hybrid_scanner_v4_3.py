...
class HybridScannerV4_3:
    ...
    def detect(self, img):
        print("\n" + "="*50)
        print("  Hybrid Scanner V4_3 (Enhanced OpenCV Engine)")
        print("="*50)
        h, w = img.shape[:2]
    ...
    def _validate_corner_by_harris_neighbors(self, harris_pts, candidate_pt, corner_type, 
                                              ...
                                              gap_tol=0.55, excluded_pts=None):
        """
        验证一个候选点是否是合法的棋盘角。
    ...
    def _is_noise_point(self, img, suspect_pt, all_harris_global, expected_gap, 
                        angle1, angle2, patch_center, gap_tol=0.55, verify_roi_r=280,
                        excluded_pts=None):
        """
        验证一个疑似噪点是否真的是噪点。
    ...
    def exact_recognize_corner(self, img, approx_pt, corner_type, angle1, angle2, expected_gap, debug_canvas=None):
        ...
        step = 30
        backtrack_count = 0
        MAX_BACKTRACK = 8  # 增加backtrack最大次数给予更多机会
        ...
        for k in range(15):  # 增加总体探路步数上限以应对复杂的寻找
            ...
            if label == 'Edge':
                ...
                # ★ 关键修复：如果下一步和上一步完全一样，说明在来回震荡，强制backtrack
                next_pos = (int(next_cx), int(next_cy))
                curr_pos = (int(cx), int(cy))
                if next_pos == curr_pos or next_pos in visited_positions:
                    backtrack_count += 1
                    if backtrack_count > MAX_BACKTRACK:
