...
class HybridScannerV4:
    ...
    def detect(self, img):
        print("\n" + "="*50)
        print("  Hybrid Scanner V4 (Enhanced OpenCV Engine)")
        print("="*50)
        h, w = img.shape[:2]
    ...
    def _snap_to_best_harris_corner(self, roi, hough_pt, x1, y1, corner_type,
                                     angle1, angle2, expected_gap, search_r=80):
        """
        在Hough交点附近，找所有Harris候选点，
        用网格拓扑验证，选出得分最高的作为最终角点。
        """
        try:
            gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
            ...
            # 将相邻的 Harris 像素簇聚合成离散的角点 (Centroids)
            ...
            # centroids[1:] 排除背景 0，得到每个“蓝点”的几何中心
            all_harris_global = [(int(c[0]) + x1, int(c[1]) + y1) for c in centroids[1:]]
        except Exception as e:
            print(f"  [Harris Topo] Preprocessing failed: {e}")
            return hough_pt

        if len(all_harris_global) == 0:
            return hough_pt
        
        print(f"  [Harris List] Detected {len(all_harris_global)} points for {corner_type}: {all_harris_global}")
        ...
        if local_gap is None:
            ...
            print(f"  [Harris Topo] Estimated local gap from {len(all_harris_global)} pts: {local_gap:.1f}px (global average was {expected_gap:.1f}px)")

        for cand in candidates:
            is_valid, score = self._validate_corner_by_harris_neighbors(
                all_harris_global, cand, corner_type, expected_gap=local_gap, angle1=angle1, angle2=angle2
            )
        ...
        if best_pt is None:
            print(f"  [Harris Topo] No valid topo candidate, fallback to Hough intersection")
            return hough_pt

        dist = np.sqrt((best_pt[0]-hx)**2 + (best_pt[1]-hy)**2)
        print(f"  [Harris Topo] Validated: {hough_pt} -> {best_pt} (score={best_score:.3f}, dist={dist:.1f}px)")
        return best_pt
