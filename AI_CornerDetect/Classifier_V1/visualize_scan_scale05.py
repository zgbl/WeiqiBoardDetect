...
def visualize_scan_scale05():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")

    # 路径配置
    INPUT_DIR = Path(r"E:\Data\WeiqiPics\Test1")
    OUTPUT_DIR = Path(r"E:\Data\WeiqiPics\Test1\output_scale05")
    ...
    crop_size = 256
    target_size = 128
    stride = 128 # 步长也相应调大一点点，或者维持 64 以保证覆盖
    ...
    print(f"Scanning with Crop={crop_size} (Scale 0.5) for {len(img_files)} images.")
    ...
    for img_path in tqdm(img_files):
        ...
        for y in range(half_crop, h - half_crop, stride):
            for x in range(half_crop, w - half_crop, stride):
                ...
                # 4. 在原图上标记 (位置依然是中心点 x, y)
                if conf > 0.8:
                    if pred_id == 0: # Corner
                        cv2.putText(display_img, "C", (x - 15, y + 15), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 4)
                    elif pred_id == 1: # Edge
                        cv2.putText(display_img, "E", (x - 15, y + 15), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)

        # 保存结果
        save_path = OUTPUT_DIR / f"scan_s05_{img_path.name}"
        cv2.imwrite(str(save_path), display_img)

    print(f"\nScale 0.5 scan complete! Check results in: {OUTPUT_DIR}")

if __name__ == "__main__":
    visualize_scan_scale05()
