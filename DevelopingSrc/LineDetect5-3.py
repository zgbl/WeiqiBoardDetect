...
def regularize_board_lines(h_lines, v_lines, n_lines=9):
    ...
    if h_lines:
        ...
        if len(h_positions) >= 2:
            ...
            # 如果只有一条横线，直接使用原线条
            regularized_h_lines = h_lines.copy()
    ...
    if v_lines:
        ...
        if len(v_positions) >= 2:
            ...
            if h_lines:
                ...
                y_min = int(min(h_positions))
                y_max = int(max(h_positions))
            else:
                # 如果没有横线，使用竖线自身的范围
                all_y = []
                for x1, y1, x2, y2 in v_lines:
            ...
            # 如果只有一条竖线，直接使用原线条
            regularized_v_lines = v_lines.copy()
