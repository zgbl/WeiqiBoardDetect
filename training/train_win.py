...
class PatchDataset(Dataset):
    ...
    def __init__(self, patches_dirs, gomrade_dirs=None, split="train", transform=None,
        ...
        # Get all session names from gomrade directories to prevent data leakage
        # If gomrade_dirs is not provided, we will group by filename prefix (less precise)
        session_names = set()
        if gomrade_dirs:
            if not isinstance(gomrade_dirs, list):
                gomrade_dirs = [gomrade_dirs]
            for g_dir in gomrade_dirs:
                g_path = Path(g_dir)
                if g_path.exists():
                    # Each subdirectory in gomrade_dir is a session
                    for sub in g_path.iterdir():
                        if sub.is_dir() and sub.name != "patches":
                            session_names.add(sub.name)

        all_files = [] # List of (path, label, session_name)
        
        # 1. Collect all patches
        for p_dir in self.patches_dirs:
            if not p_dir.exists():
                print(f"  [WARNING] Patches directory not found: {p_dir}")
                continue
            ...
            for cls_name, cls_idx in self.CLASS_TO_IDX.items():
                ...
                for f_path in files:
                    fname = f_path.name
                    # Identify session: match against known session names (longest first)
                    matching_session = "unknown"
                    if session_names:
                        # Sort session names by length descending to match longest possible prefix
                        sorted_sessions = sorted(list(session_names), key=len, reverse=True)
                        for s_name in sorted_sessions:
                            if fname.startswith(s_name + "_"):
                                matching_session = s_name
                                break
                    else:
                        # Fallback: part before the last three underscores (frame_rXX_cYY)
                        # Pattern is session_frame_rXX_cYY.jpg
        ...
        # 3. Split sessions (Data Leakage Fix)
        ...
        selected_sessions = []
        if split == "train":
            selected_sessions = train_sessions
        elif split == "val":
            selected_sessions = val_sessions
        elif split == "test":
            selected_sessions = test_sessions
        else:
            selected_sessions = unique_sessions # All
        ...
        if split == "train" and 0.0 < subset_ratio < 1.0:
            ...
            print(f"  [SUBSET] Taking random {k:,} samples from {len(train_sessions)} sessions")
        
        # Calculate class counts for sampler/logging
        counts = [0, 0, 0]
        ...
        # Print info
        print(f"  [{split:5s}] Sessions={len(selected_sessions)}/{n_sess}  Total={len(self.samples):,}  "
...
def get_white_area_ratio(imgs_tensor):
    """
    Calculate the ratio of 'white' pixels in the image.
    Standard Normalization: (x - mean) / std
    We must UN-NORMALIZE before brightness check to be lighting-robust.
    """
    # imgs shape: [B, 3, H, W]
    # Standard ImageNet normalization params
    ...
    # Undo normalization: x_raw = x_norm * std + mean
    unnorm = imgs_tensor * std + mean
    
    # Brightness heuristic: average of R, G, B channels in [0, 1] space
    brightness = unnorm.mean(dim=1)  # [B, H, W]
    
    # Thresholding: pixels with raw brightness > 0.7
    white_mask = (brightness > 0.7).float()
    
    # Ratio of white pixels in each image
    ratios = white_mask.mean(dim=(1, 2))  # [B]
    return ratios
...
def evaluate(model, loader, criterion, device, desc="  Val  ", heuristic_cfg=None):
    ...
    # Heuristic config
    use_filter = heuristic_cfg.get("white_area_filter", False) if heuristic_cfg else False
    min_ratio = heuristic_cfg.get("min_area_ratio", 0.10) if heuristic_cfg else 0.10
    ...
    for imgs, labels in pbar:
        ...
        # --- Apply size-based heuristic for White stones (W=index 1) ---
        if use_filter:
            ratios = get_white_area_ratio(imgs)
            # If predicted as White but white area is too small, revert to Empty (E=index 2)
            mask_is_w = (preds == 1)
            mask_too_small = (ratios < min_ratio)
            preds[mask_is_w & mask_too_small] = 2
...
Example:
  ...
  python train.py --sampling sqrt               # Modify sampling strategy
    ...
    patches_raw   = args.patches or gp(cfg, "data", "patches_dir", default="patches")
    if isinstance(patches_raw, list):
        patches_dirs = [Path(p) for p in patches_raw]
    else:
        patches_dirs = [Path(patches_raw)]
    ...
    print(f"  Patch Dirs: {[str(p) for p in patches_dirs]}")
    ...
    # Get gomrade_dirs from config for session identification
    gomrade_dirs = gp(cfg, "data", "gomrade_dir")

    train_ds = PatchDataset(patches_dirs, gomrade_dirs, "train", train_transforms, val_ratio, test_ratio, args.seed, subset_ratio)
    val_ds   = PatchDataset(patches_dirs, gomrade_dirs, "val",   val_transforms,   val_ratio, test_ratio, args.seed)
    test_ds  = PatchDataset(patches_dirs, gomrade_dirs, "test",  val_transforms,   val_ratio, test_ratio, args.seed)
    ...
    # -- Loss Function --
    # Simplified: Removed class_weights to prevent interference with WeightedRandomSampler
    # as suggested in optimization review. The sampler already handles balance.
    criterion = nn.CrossEntropyLoss()

    # -- Optimizer + LR Scheduler --
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_dec)
    
    # Fix: Correct T_max for CosineAnnealing when resuming
