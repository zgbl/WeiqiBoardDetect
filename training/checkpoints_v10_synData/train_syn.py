"""
train_syn.py — Training Pipeline for Synthetic Data
================================================================
Upgrades based on train_win_ram.py, but adapted for synthetic patches:
  ✅ Full RAM Pre-loading with Memory Safety (.copy())
  ✅ Auto-Cleaning: Saves 'suspects.csv' (pred != label & conf > 0.9)
  ✅ Data Filtering: Skipped for synthetic data (controlled by config)
  ✅ Early Stopping (Patience=10)
  ✅ Windows Stability Fix (num_workers=0)
  ✅ Random split (no session isolation needed for synthetic data)
"""
...
class PatchDataset(Dataset):
    ...
    def __init__(self, patches_dirs, split="train", transform=None,
                 val_ratio=0.15, test_ratio=0.05, seed=42, subset_ratio=1.0, 
                 preload_to_ram=True, white_area_filter=False):
        ...
        for p_dir in self.patches_dirs:
            ...
            for cls_name, cls_idx in self.CLASS_TO_IDX.items():
                ...
                for f_path in files:
                    all_files.append((str(f_path), cls_idx))
        
        if not all_files: raise RuntimeError(f"No patches found in {self.patches_dirs}!")

        # 2. Random Split (File level)
        ...
        if split == "train":
            temp_samples = all_files[:n_train]
        elif split == "val":
            temp_samples = all_files[n_train:n_train + n_val]
        elif split == "test":
            temp_samples = all_files[n_train + n_val:]
        else:
            temp_samples = all_files
...
# --- Main ---
def main():
    parser = argparse.ArgumentParser(description="Synthetic Weiqi Training Pipeline")
    parser.add_argument("--config", default="config_syn.yaml")
    ...
    output_dir = Path(gp(cfg, "model", "output_dir", default="checkpoints_syn"))
    ...
    num_workers = 0 if not args.no_ram else gp(cfg, "train", "num_workers", default=0)
    sampling = gp(cfg, "train", "sampling_strategy", default="sqrt")
    use_h_filter = gp(cfg, "heuristic", "white_area_filter", default=False)

    device = detect_device()
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = Path(gp(cfg, "logging", "log_dir", default="log_syn")) / f"train_syn_{ts}.log"
    tee = TeeLogger(log_path)
    sys.stdout = tee; sys.stderr = tee
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n[SYNTHETIC MODE] Arch: {arch} | Epochs: {epochs} | P-Size: {patch_size}")
    print(f"Memory Safety Enabled | W-Area Filtering: {use_h_filter}")
    ...
    train_ds = PatchDataset(patches_dirs, "train", train_tf, preload_to_ram=not args.no_ram, white_area_filter=use_h_filter)
    val_ds = PatchDataset(patches_dirs, "val", val_tf, preload_to_ram=not args.no_ram, white_area_filter=False)
