...
cv2.imshow("原始图片", original_img)
cv2.moveWindow("原始图片", 0, 0)
...
cv2.imshow('改进边缘检测', im_edge_improved)
cv2.moveWindow("改进边缘检测", 300, 0)
...
if rect is None:
    ...
    cv2.imshow('原始Canny + 轻微膨胀', im_edge_dilated)
    cv2.moveWindow("原始Canny + 轻微膨胀", 600, 0)
...
if rect is None:
    ...
    cv2.imshow('自动检测到的角点', corner_img)
    cv2.moveWindow("自动检测到的角点", 900, 0)
    ...
    cv2.imshow('原始图像上的棋盘边界', debug_original_with_rect)
    cv2.moveWindow('原始图像上的棋盘边界', 1200, 0)
    ...
    cv2.imshow('透视校正后的棋盘', img_processed)
    cv2.moveWindow("透视校正后的棋盘", 1500, 0)
    print("透视变换完成！")
...
cv2.imshow('霍夫变换边缘图', edges)
cv2.moveWindow('霍夫变换边缘图', 1800, 0)
...
cv2.imshow('所有检测到的交点', grid_img)
cv2.moveWindow('所有检测到的交点', 2100, 0)
...
if len(intersections) > intersection_threshold_val:
    ...
    if len(valid_rows) >= 18: # 允许少量误差
        ...
        circles = cv2.HoughCircles(gray_for_hough, method=cv2.HOUGH_GRADIENT,
        ...
        if circles is not None:
            ...
            cv2.imshow('最终围棋棋盘', final_img)
            cv2.moveWindow('最终围棋棋盘', 2400, 0)
        ...
        cv2.imshow('最终围棋棋盘', img_processed) # 使用 img_processed
        cv2.moveWindow('最终围棋棋盘', 2400, 0)
...
if 'grid_xy' in locals() and 'stones' in locals() and rect is not None:
    try:
        ...
        cv2.imshow('只包含棋盘和棋子的新图像', board_only_img)
        cv2.moveWindow('只包含棋盘和棋子的新图像', 2700, 0)
